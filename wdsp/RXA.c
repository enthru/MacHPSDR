...
void create_rxa (int channel)
{
	...
	rxa[channel].wfmd.p = create_wfmd (
		...
		rxa[channel].midbuff,							// pointer to output buffer
		ch[channel].dsp_rate,							// sample rate
		75000.0,										// deviation (peak, for normalisation)
		0.0,											// audio f_low
		15000.0,										// audio f_high
		0.5,											// audio gain (headroom so loud peaks don't clip)
		50.0e-6,										// de-emphasis tau (50 us, EU)
		2048,											// # coefs for audio cutoff filter
		0);												// min phase flag for audio cutoff filter
	...
	// NR3: RNNoise-based noise reduction
