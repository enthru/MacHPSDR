...
# behind a byte counter that went up regardless -- were both invisible from
# outside, and the cure adds three more properties nothing else can see (stop
# drains the queue, an overflow costs frames and not the file, a failed write
# stops the recording).  gtk4 is in the link only because recorder.c reaches
# <gtk/gtk.h> through radio.h; the application surface is stubbed in the
# harness, as in cw_offline.
...
app: $(PROGRAM)
	...
	@echo '# Finder launches have no terminal. Keep the latest run for startup diagnostics.' >> $(APP_BUNDLE)/Contents/MacOS/$(APP_NAME)
	@echo 'if [ ! -t 2 ]; then' >> $(APP_BUNDLE)/Contents/MacOS/$(APP_NAME)
	@echo '  _logdir="$$HOME/Library/Logs/MacHPSDR"' >> $(APP_BUNDLE)/Contents/MacOS/$(APP_NAME)
	@echo '  if mkdir -p "$$_logdir" && touch "$$_logdir/startup.log"; then' >> $(APP_BUNDLE)/Contents/MacOS/$(APP_NAME)
	@echo '    exec >"$$_logdir/startup.log" 2>&1' >> $(APP_BUNDLE)/Contents/MacOS/$(APP_NAME)
	@echo '  fi' >> $(APP_BUNDLE)/Contents/MacOS/$(APP_NAME)
	@echo '  unset _logdir' >> $(APP_BUNDLE)/Contents/MacOS/$(APP_NAME)
	@echo 'fi' >> $(APP_BUNDLE)/Contents/MacOS/$(APP_NAME)
	@echo 'echo "launcher: $$(date -u +%Y-%m-%dT%H:%M:%SZ) $$DIR" >&2' >> $(APP_BUNDLE)/Contents/MacOS/$(APP_NAME)
	...
	@echo 'echo "launcher: executing binary at unix $$(date +%s)" >&2' >> $(APP_BUNDLE)/Contents/MacOS/$(APP_NAME)
